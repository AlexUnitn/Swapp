...
describe('Booking API', () => {
    describe('GET /api/booking/user/:id', () => {
        test('deve restituire i booking per utente', async () => {
            const owner = await createUser()
            const borrower = await createUser()
            const token = createToken(borrower)
            const item = await createItem(owner._id)
            await createBooking(item, borrower)
            const response = await request(app).get(`/api/booking/user/${borrower._id}`)
                .set('Authorization', `Bearer ${token}`)
            expect(response.status).toBe(200)
            expect(Array.isArray(response.body)).toBe(true)
        })

        test('deve restituire errore per utente non valido', async () => {
            const user = await createUser()
            const token = createToken(user)
            const response = await request(app).get('/api/booking/user/000000000000000000000000')
                .set('Authorization', `Bearer ${token}`)
